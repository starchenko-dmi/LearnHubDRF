...
def test_send_course_update_notification(self, mock_send_mail):
    ...
    course = Course.objects.create(
        title="Новый курс", description="Тест", owner=user, price=1000.00
    )
...
class CeleryTasksTest(TestCase):
    ...
    def test_send_course_update_notification(self, mock_send_mail):
        ...
        user = User.objects.create_user(
            email="subscriber@example.com", password="pass123"
        )

        # 2. Создаём курс
        course = Course.objects.create(
            title="Курс Python", description="Описание", owner=user, price=1000.00
        )
